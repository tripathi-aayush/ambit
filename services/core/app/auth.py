...
Deliberately simple -- one shared secret via the X-Ambit-Key header, no
...
API_KEY_HEADER = "X-Ambit-Key"


async def require_api_key(x_ambit_key: str | None = Header(default=None, alias=API_KEY_HEADER)) -> None:
    if not settings.ambit_api_key:
        ...
        raise HTTPException(status_code=500, detail="AMBIT_API_KEY is not configured on the server")
    if x_ambit_key is None:
        raise HTTPException(status_code=401, detail="missing X-Ambit-Key header")
    ...
    if not hmac.compare_digest(x_ambit_key, settings.ambit_api_key):
        raise HTTPException(status_code=403, detail="invalid X-Ambit-Key")
