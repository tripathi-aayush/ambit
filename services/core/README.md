...
cp .env.example .env   # fill in AMBIT_API_KEY, LLM_PROVIDER + one API key
...
Every request needs an `X-Ambit-Key` header matching `AMBIT_API_KEY`, or it's rejected (`500` if unset, `401`/`403` if missing/wrong on the request). See the root README's [Authentication](../../README.md#authentication) section for the full setup and failure modes — the frontend's `apps/web/.env.local` needs the same key too.
